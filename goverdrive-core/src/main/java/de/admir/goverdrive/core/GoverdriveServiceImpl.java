...
import de.admir.goverdrive.core.cache.CacheService;
...
public class GoverdriveServiceImpl implements GoverdriveService {
   ...
   private Xor<AuthorizationError, Drive> createAuthorizedDriveService() {
      return authorize().mapRight(credential -> new Drive.Builder(HTTP_TRANSPORT, JSON_FACTORY, credential).setApplicationName(APPLICATION_NAME).build());
   }
